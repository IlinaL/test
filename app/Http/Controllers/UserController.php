...
class UserController extends Controller
{
    public function edit(Request $request){
        $userDetails = User::select('id','name','email', 'user_type')->where('id',$request->id)->first();
        return view('edit',['userDetails'=>$userDetails]);
    }

    public function update(UpdateUserRequest $request){
        $isUpdated = User::where('id',$request->id)->update(['name'=>$request->name,'email'=>$request->email,'user_type'=>$request->user_type]);
        if($isUpdated){
